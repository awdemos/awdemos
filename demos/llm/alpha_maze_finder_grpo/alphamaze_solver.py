        ...
        for i in range(self.size):
            for j in range(self.size):
                if (
                    random.random() < 0.3
                    and (i, j) != (0, 0)
                    and (i, j) != (self.size - 1, self.size - 1)
                ):
                    maze[i, j] = 1  # Wall
        return maze
...
def generate_maze_dataset(num_mazes=1000):
    ...
    for _ in range(num_mazes):
        ...
        dataset.append({"maze": maze_state, "solution": solution, "prompt": prompt})
    return dataset
...
def solve_maze(game):
    ...
    while queue:
        ...
        for dx, dy, move in [
            ...
            if (
                0 <= next_x < game.size
                and 0 <= next_y < game.size
                and game.maze[next_x, next_y] == 0
            ):
...
def maze_reward(completions, prompts, **kwargs):
    ...
    device = (
        kwargs.get("model", torch.device("cpu")).device
...
class CustomGRPOTrainer(GRPOTrainer):
    ...
    def multi_turn_generation(
        ...
        while not game.done and len(moves) < max_new_tokens:
            ...
            move = (
                tokenizer.decode(outputs[0][-1:], skip_special_tokens=True)
                .strip()
                .lower()
            )
